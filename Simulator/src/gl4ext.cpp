...
#include "../Header Files/geometryutils.h"
#include "../Header Files/dds.h"
	...
	OpenGLScreenQuad::OpenGLScreenQuad(const char* psfile, const char* defines)
{
	...
	if (psfile != nullptr) {
		GLCreateEffectFromFile("../GLSL/SHADER/screenquad.vert", 0, 0, 0, psfile, &effect, defines);  //this part is having an error causing a nullptr on ""this""
		effect->SetInt("sampler0", 0);
	}
}

*/
OpenGLScreenQuad::OpenGLScreenQuad(const char* psfile, const char* vsfile, const char* defines)
{
	effect = nullptr;
	vertexlayout = 0;
	glGenVertexArrays(1, &vertexlayout);
	...
	if (psfile != nullptr)
	{
		GLCreateEffectFromFile(vsfile, 0, 0, 0, psfile, &effect, defines);
		effect->SetInt("sampler0", 0);
	}
}
